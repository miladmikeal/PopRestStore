...
storeComps.Navbar = {
  ...
  methods: {
    ...
    getCartInfo: function() {
      ProductService.getCartInfo(this.axiosConfig).then(
        function(data) {
          ...
          for (var i = 0; i < data.orderItemList.length; i++) {
            if (data.orderItemList[i].itemTypeEnumId == "ItemProduct") {
              this.productsQuantity =
                data.orderItemList[i].quantity + this.productsQuantity;
            }
          }
        }.bind(this)
      );
    ...
    searchProduct: function() {
      location.href = "/moquicon/search/" + this.searchText;
    }
  },
  created: function() {
  ...
  mounted: function() {
    var vm = this;
    if (
      this.storeInfo.categoryByType &&
      this.storeInfo.categoryByType.PsctBrowseRoot &&
      this.storeInfo.categoryByType.PsctBrowseRoot.productCategoryId
    ) {
...
storeComps.NavbarTemplate = getPlaceholderRoute(
  "template_client_header",
  "Navbar",
  storeComps.Navbar.props
);
...
storeComps.FooterPageTemplate = getPlaceholderRoute(
  "template_client_footer",
  "FooterPage",
  storeComps.FooterPage.props
);
...
storeComps.MenuLeftTemplate = getPlaceholderRoute(
  "template_client_menu",
  "MenuLeft",
  storeComps.MenuLeft.props
);
...
storeComps.ModalAddress = {
  ...
  methods: {
    getRegions: function(geoId) {
      GeoService.getRegions(geoId).then(
        function(data) {
    ...
    addCustomerShippingAddress: function() {
      var error = false;
      if (
        this.shippingAddress.toName == null ||
      ...
      if (
        this.shippingAddress.stateProvinceGeoId == null ||
        this.shippingAddress.stateProvinceGeoId.trim() === ""
      ) {
      ...
      if (
        this.shippingAddress.contactNumber == null ||
        this.shippingAddress.contactNumber.trim() === ""
      ) {
  ...
  mounted: function() {
    ...
    $("#addressFormModal").on("show.bs.modal", function(e) {
      vm.reset();
    });
  }
};
storeComps.ModalAddressTemplate = getPlaceholderRoute(
  "template_client_modalAddress",
  "ModalAddress",
  storeComps.ModalAddress.props
);
  ...
  methods: {
    ...
    selectBillingAddress: function(address) {
      if (address == "NEW_ADDRESS") {
        ...
        this.paymentMethod.stateProvinceGeoId =
          address.postalAddress.stateProvinceGeoId;
        this.responseMessage = "";
      }
    ...
    addCustomerPaymentMethod: function(event) {
      ...
      if (
        this.paymentMethod.titleOnAccount == null ||
      ...
      this.disabled = true;
      CustomerService.addPaymentMethod(this.paymentMethod, this.axiosConfig)
        .then(
          function(data) {
            this.responseMessage = "";
            this.completeCallback(data);
...
storeComps.ModalCreditCardTemplate = getPlaceholderRoute(
  "template_client_modalCreditCard",
  "ModalCreditCard",
  storeComps.ModalCreditCard.props
);
